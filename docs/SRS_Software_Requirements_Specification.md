...
| **Modified Dijkstra** | **Shortest-path** algorithm on a graph; "modified" by using **edge weight = distance + (risk × 500)** so the path minimizes risk-weighted cost rather than pure distance. **Dijkstra** is standard for non-negative weights and gives the single optimal path; the system runs it up to 3 times with edge penalties to produce up to 3 distinct safe routes. Basis: graph theory and routing literature; risk multiplier 500 is a design choice to heavily favor safety over distance. |
